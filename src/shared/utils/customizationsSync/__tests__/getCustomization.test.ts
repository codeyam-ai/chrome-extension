...
jest.mock(
    '_src/shared/utils/customizationsSync/accountCustomizationEncryption',
    () => ({
...
describe('getCustomization', () => {
    test('returns decrypted account customization when explorerApiCall returns status 200 and json object', async () => {
        ...
        const mockDecryptedAccountCustomization = {
            accountInfo: 'test-account-info',
        };
        (explorerApiCall as jest.Mock).mockResolvedValue({
            json: mockJson,
            status: 200,
        });
        (decryptAccountCustomization as jest.Mock).mockReturnValue(
            mockDecryptedAccountCustomization
        );
        const result = await getCustomization(jwt, privateKey);
        expect(result).toEqual(mockDecryptedAccountCustomization);
        expect(explorerApiCall).toHaveBeenCalledWith(
            'v1/user/profile',
            'GET',
            jwt
        );
        expect(decryptAccountCustomization).toHaveBeenCalledWith(
            mockJson.data,
            privateKey
        );
    });
...
it('handles timeout from explorerApiCall', async () => {
    (explorerApiCall as jest.Mock).mockImplementation(
        () =>
            new Promise((resolve) => {
