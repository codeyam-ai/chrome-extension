...
describe('WindowMessageStream', () => {
    ...
    beforeEach(() => {
        ...
        messageEvent = new MessageEvent('message', {data: {target: 'ethos_in-page', payload: {}}});
    });

    test('constructor throws an error if name and target are the same', () => {
        expect(() => new WindowMessageStream('ethos_in-page', 'ethos_in-page')).toThrowError('[WindowMessageStream] name and target must be different');
    });
