    ...
    <Form className={st.container} autoComplete="off" noValidate={true}>
      <div className={st.group}>
        ...
        <div className={st.muted}>
          Available balance:{" "}
          {intl.formatNumber(BigInt(coinBalance), balanceFormatOptions)}{" "}
          {coinSymbol}
        </div>
        <ErrorMessage className={st.error} name="amount" component="div" />
      </div>
      <div className={st.group}>
      ...
      {submitError ? (
        <div className={st.group}>
          <Alert>
            <strong>Stake failed.</strong> <small>{submitError}</small>
          </Alert>
        </div>
      ) : null}
      <div className={st.group}>
