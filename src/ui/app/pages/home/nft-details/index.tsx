      ...
      <BottomMenuLayout>
        <Content>
          <section className={st.nftDetail}>
            <NFTDisplayCard nftobj={nft} size="large" expandable={true} />
            {NFTDetails}
          </section>
        </Content>
...
function NFTDetailsPage() {
  ...
  const objectId = useMemo(() => searchParams.get("objectId"), [searchParams]);
  ...
  return (
    <Loading loading={loadingBalance}>
      {objectId && startNFTTransfer ? (
        ...
        <NFTdetailsContent nft={activeNFT} onClick={startNFTTransferHandler} />
      )}
    </Loading>
  );
}
