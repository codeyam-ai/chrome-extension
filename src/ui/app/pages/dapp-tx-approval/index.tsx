    ...
    () =>
      txRequest?.type === "move-call"
        ? [
            {
              label: "Package",
              content: truncateMiddle(txRequest.tx.packageObjectId, 12),
        ...
        >
          {valuesContent.map(({ label, content }) => (
            <Fragment key={label}>
              <div className="flex justify-between">
                ...
                <div className={st.value + " dark:text-gray-400"}>
                  {content}
                </div>
              </div>
            </Fragment>
          ...
          <div className="">
            ...
            <div className={details ? "" : "hidden"}>
              {detailedValuesContent.map(({ label, content, title }) => (
                <Fragment key={label}>
                  <div className="flex justify-between">
                    <label className="text-gray-500 dark:text-gray-400 text-sm">
                      {label}
                    </label>
                    <div className={st.value} title={title}>
                      {content}
                    </div>
                  </div>
                </Fragment>
