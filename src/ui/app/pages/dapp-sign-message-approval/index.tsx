  ...
  const signMessageRequestSelector = useMemo(
    () => (state: RootState) =>
      (signMessageRequestID &&
        signMessageRequestsSelectors.selectById(state, signMessageRequestID)) ||
      null,
    [signMessageRequestID]
  );
      ...
      {signMessageRequest && (
        ...
        >
          <div className="flex gap-3 justify-center items-center py-6">
            <img src={signMessageRequest.originFavIcon} alt="Site Favicon" />
            <div>
              <div className="text-base">{signMessageRequest.origin}</div>
