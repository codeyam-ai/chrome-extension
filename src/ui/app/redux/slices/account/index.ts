    ...
    if (authentication) {
      ...
      if (authentication !== AUTHENTICATION_REQUESTED) {
        ...
        activeAccountIndex = parseInt(
          (await getEncrypted("activeAccountIndex", authentication)) || "0"
        );
...
export const saveAccountInfos = createAsyncThunk(
  ...
  async (accountInfos: AccountInfo[], { getState }): Promise<AccountInfo[]> => {
    const {
      account: { passphrase },
    } = getState() as RootState;
...
const accountSlice = createSlice({
  ...
  extraReducers: (builder) =>
    builder
      .addCase(loadAccountInformationFromStorage.fulfilled, (state, action) => {
        state.loading = false;
        ...
        state.activeAccountIndex = action.payload.activeAccountIndex || 0;

        state.address =
          state.accountInfos.find(
            (accountInfo) =>
              (accountInfo.index || 0) === state.activeAccountIndex
          )?.address || null;
      })
