...
export class Ticket {
    ...
    public static async lookupTicketRecord(provider: JsonRpcProvider, address: string, ticketId: string) {
        const ticketResponse = await provider.devInspectMoveCall(address,
            {
                packageObjectId: ticket,
                module: 'token_gated_ticket',
                function: 'get_ticket_record_by_ticket_id',
                typeArguments: [CAPY_TYPE],
                arguments: [TICKET_AGENT_ID, ticketId],    
            }
        )
        ...
        if ("Ok" in ticketResponse.results) {
            ...
            const dataNumberArray = ticketResponse.results.Ok[0][1].returnValues?.[0]?.[0]
