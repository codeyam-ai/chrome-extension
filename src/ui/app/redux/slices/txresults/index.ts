...
>(
  ...
  async (
          ...
          txEffs
            .map((txEff, i) => {
              const [seq, digest] = transactions.filter(
                (transactionId) =>
                  transactionId[1] === getTransactionDigest(txEff.certificate)
              )[0];
              ...
              const recipient =
                tranferSui?.recipient ?? txTransferObject?.recipient;
              ...
              return {
                ...
                ...(tranferSui?.amount ? { amount: tranferSui.amount } : {}),
                ...(recipient
                  ? {
                      to: recipient,
                    }
    ...
    // remove duplicates
    const objectIDs = [
      ...new Set(resp.filter((itm) => itm).map((itm) => itm?.objectId || "")),
    ];
    ...
    const txnResp = resp.map((itm) => {
      const objectTxObj =
        txObjects && itm?.objectId && Array.isArray(txObjects)
          ? txObjects.find((obj) => obj.reference.objectId === itm.objectId)
          : null;
