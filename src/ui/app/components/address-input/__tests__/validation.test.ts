...
describe('Address Input Validation', () => {
    ...
    it('Should validate a correct address', async () => {
        ...
        await expect(SUI_ADDRESS_VALIDATION.validate('0x123456789abcdef')).resolves.not.toThrow();
    });
    
    it('Should not validate an incorrect address', async () => {
        mockIsValidSuiAddress.mockReturnValue(false);
        await expect(SUI_ADDRESS_VALIDATION.validate('incorrect')).rejects.toMatchObject({message: 'Invalid address. Please check again.'});
    });
    
    it('Should not validate an empty value', async () => {
        await expect(SUI_ADDRESS_VALIDATION.validate('')).rejects.toMatchObject({message: "Recipient's address or SuiNS Name is a required field"});
    });
