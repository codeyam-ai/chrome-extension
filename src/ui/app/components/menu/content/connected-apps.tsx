...
export default function ConnectedApps() {
  ...
  useEffect(() => {
    const getPermissions = async () => {
      const permissions = Object.values(await Permissions.getPermissions());
  ...
  useEffect(() => {
    const groupPermissions = async () => {
      ...
      const _groupedPermissions = permissions.reduce((groups, permission) => {
              ...
              <div className="flex-1 flex flex-col w-1/2 truncate text-sm">
                {permission.title && (
                  <a
                    ...
                    rel="noreferrer"
                  >
                    {truncateString(formatUrl(permission.title), 24)}
                  </a>
                )}
                <a
                  href={permission.origin}
      ...
      <div key={`wallet-connections-${name}`} className="mb-12">
        ...
        <ul className="grid grid-cols-1 gap-2">
          {permissions.map((permission: Permission, index: number) => (
            <PermissionRevoke key={`revoke-${index}`} permission={permission} />
          ))}
        </ul>
      </div>
    );
