...
export default function SwitchWallet() {
  ...
  const _createWallet = useCallback(() => {
    ...
    const loadAccFromStorage = async () => {
      const sortedAccountIndices = accountInfos.map((a) => a.index || 0).sort();
      const nextAccountIndex =
        +sortedAccountIndices[sortedAccountIndices.length - 1] + 1;

      let newAccountInfos: AccountInfo[];
      if (authentication) {
        const newAccount = await Authentication.createAccount(nextAccountIndex);
        ...
        newAccountInfos = [
          ...accountInfos,
          {
            ...
            address: keypairVault.getAddress(nextAccountIndex) || "",
            seed: (keypairVault.getSeed(nextAccountIndex) || "").toString(),
          },
        ];
            ...
            {accountInfos.map((accountInfo, key) => {
              return (
                <li key={key} className="col-span-1 flex rounded-md shadow-sm">
                  <AddressButton
                    info={accountInfo}
                    isActive={(accountInfo.index || 0) === activeAccountIndex}
